...
const paperMenu = (id) => { const token = (paperTokenCounter++).toString(36); paperTokens.set(token, id); if (paperTokens.size > 1000) paperTokens.delete(paperTokens.keys().next().value); return new InlineKeyboard().text('تحديث السعر وPnL', `p:${token}`).row().text('بيع محاكاة 50٪', `ps:${token}:0.5`).text('بيع محاكاة 100٪', `ps:${token}:1`); };
...
const panelKeyboard = (s) => new InlineKeyboard().text(s.autoSniperEnabled ? 'إيقاف القنص والشراء' : 'تشغيل القنص تلقائياً', `panel:${s.autoSniperEnabled ? 'stop' : 'start'}`).row().text('ضبط الإعدادات', 'panel:settings').text('تحديث', 'panel:refresh').row().text('تفاصيل العمليات', 'panel:details').row().text('إيقاف الشراء فقط', 'panel:buyoff').text('تشغيل البيع', 'panel:sellon');
...
bot.callbackQuery(/^panel:(start|stop|buyoff|sellon|refresh|settings|details|back)$/, async (ctx) => { if (!isAdmin(ctx)) return ctx.answerCallbackQuery({ text: 'للمشرف فقط' }); const action = ctx.match[1]; if (action === 'refresh') return ctx.answerCallbackQuery({ text: 'اللوحة محدثة' }); await ctx.answerCallbackQuery({ text: 'تم' }); const s = settingsForAdmin(); s.paperPanelChatId = String(ctx.callbackQuery.message.chat.id); s.paperPanelMessageId = ctx.callbackQuery.message.message_id; if (action === 'start') { s.autoSniperEnabled = true; s.paperTradingEnabled = true; watcher.updateSettings(s); watcher.start(); } if (action === 'stop') { s.autoSniperEnabled = false; s.paperTradingEnabled = false; watcher.updateSettings(s); watcher.stop(); } if (action === 'buyoff') { s.paperTradingEnabled = false; } if (action === 'sellon') { s.paperTradingEnabled = true; } saveSettings(config.adminId, s, config.encryptionKey); try { if (action === 'settings') return await ctx.editMessageText('ضبط القنص التجريبي\nاختر الإعداد المطلوب:', { reply_markup: settingsKeyboard() }); if (action === 'details') return await ctx.editMessageText(detailsText(s), { reply_markup: new InlineKeyboard().text('رجوع', 'panel:back') }); if (action === 'back') return await ctx.editMessageText(panelText(s), { reply_markup: panelKeyboard(s) }); await ctx.editMessageText(panelText(s), { reply_markup: panelKeyboard(s) }); } catch (error) { if (!String(error.message).includes('message is not modified')) console.error(`Panel button error: ${error.message}`); } });
bot.callbackQuery(/^cfg:(allocation|daily|profit|sizing|confirm)$/, async (ctx) => { await ctx.answerCallbackQuery(); if (!isAdmin(ctx)) return; const page = ctx.match[1]; if (page === 'allocation') return ctx.editMessageText('اختر نسبة الصفقة من رأس المال الافتراضي 1 SOL:', { reply_markup: allocationKeyboard() }); if (page === 'daily') return ctx.editMessageText('اختر الحد الأقصى للصفقات في اليوم:', { reply_markup: dailyKeyboard() }); if (page === 'profit') return ctx.editMessageText('اختر نسبة الربح التي عندها يتم البيع التلقائي:', { reply_markup: profitKeyboard() }); if (page === 'sizing') return ctx.editMessageText('اختر طريقة حساب رأس المال:', { reply_markup: sizingKeyboard() }); const s = settingsForAdmin(); s.autoSniperEnabled = true; s.paperTradingEnabled = true; s.paperPanelChatId = String(ctx.chat.id); s.paperPanelMessageId = ctx.callbackQuery.message.message_id; saveSettings(config.adminId, s, config.encryptionKey); watcher.updateSettings(s); watcher.start(); await ctx.editMessageText(panelText(s), { reply_markup: panelKeyboard(s) }); });
...
async function monitorPaperPositions() {
  const s = settingsForAdmin();
  if (paperMonitorBusy || effectiveLiveTrading(s)) return;
