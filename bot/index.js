...
const applyStrategyMode = (s, mode) => { const profiles = { ...(s.filterProfiles || {}) }; profiles[s.strategyMode || 'new'] = profileSnapshot(s); const target = profiles[mode] || (mode === 'growing' ? growingProfileDefaults : profiles.new || {}); Object.assign(s, target); s.filterProfiles = { ...profiles, [mode]: profileSnapshot({ ...s, ...target }) }; s.strategyMode = mode; return s; };
