...
const panelKeyboard = (s) => new InlineKeyboard()
  ...
  .text(s.liveTrading ? '🛡️ إيقاف الحقيقي' : '⚠️ تشغيل الحقيقي', `panel:${s.liveTrading ? 'liveoff' : 'liveon'}`).row()
...
const settingsKeyboard = () => new InlineKeyboard().text('حجم الصفقة', 'cfg:allocation').row().text('عدد الصفقات اليومية', 'cfg:daily').row().text('هدف الربح والبيع', 'cfg:profit').row().text('نمط حجم الصفقة', 'cfg:sizing').row().text('🆕 فلاتر العملات المولودة', 'filters:new').row().text('📈 فلاتر العملات بعد الولادة', 'filters:growing').row().text('♻️ الإعدادات الافتراضية', 'settings:reset:confirm').row().text('رجوع للوحة', 'panel:back');
const pendingFilters = new Map();
const strategyProfileFields = ['minCurveProgress','maxCurveProgress','minVolumeUsd','allowZeroVolume','allowZeroVolumeMinLiq','minUniqueBuyers','maxCreatorHoldingsPct','maxTopHoldersPct','requireSocialLinks','requireRenouncedAuthorities','requireBuyVolumeDominance','minLiquiditySol','minLiquidityUsd','minDexBuys','minBuySellRatio','minPriceChange1hPct','maxMarketCapUsd','minMarketCapUsd','minTokenAgeSec','maxTokenAgeSec','watchlistMinutes','paperStopLossPct','rugProtectionEnabled','capitalProtectionEnabled','capitalProtectionSellPct','capitalProtectionTriggerPct','maxHoldTimeMin'];
const newbornProfileDefaults = { minCurveProgress: 0, maxCurveProgress: 60, minVolumeUsd: 250, allowZeroVolume: true, allowZeroVolumeMinLiq: 30, minUniqueBuyers: 1, maxCreatorHoldingsPct: 10, maxTopHoldersPct: 20, requireSocialLinks: false, requireRenouncedAuthorities: false, requireBuyVolumeDominance: false, minLiquiditySol: 30, minLiquidityUsd: 0, minDexBuys: 0, minBuySellRatio: 0, minPriceChange1hPct: 0, maxMarketCapUsd: 30000, minMarketCapUsd: 0, minTokenAgeSec: 15, maxTokenAgeSec: 600, watchlistMinutes: 10, paperStopLossPct: 15, rugProtectionEnabled: true, capitalProtectionEnabled: true, capitalProtectionSellPct: 50, capitalProtectionTriggerPct: 10, maxHoldTimeMin: 0 };
const growingProfileDefaults = { minCurveProgress: 0, maxCurveProgress: 100, minVolumeUsd: 10000, allowZeroVolume: false, allowZeroVolumeMinLiq: 0, minUniqueBuyers: 10, maxCreatorHoldingsPct: 20, maxTopHoldersPct: 35, requireSocialLinks: false, requireRenouncedAuthorities: false, requireBuyVolumeDominance: true, minLiquiditySol: 0, minLiquidityUsd: 5000, minDexBuys: 25, minBuySellRatio: 1.1, minPriceChange1hPct: 0, maxMarketCapUsd: 10000000, minMarketCapUsd: 10000, minTokenAgeSec: 300, maxTokenAgeSec: 21600, watchlistMinutes: 30, paperStopLossPct: 15, rugProtectionEnabled: true, capitalProtectionEnabled: true, capitalProtectionSellPct: 50, capitalProtectionTriggerPct: 10, maxHoldTimeMin: 0 };
...
const filterKeyboard = (s) => {
  const keyboard = new InlineKeyboard();
  keyboard.text(`${modeLabels[s.strategyMode || 'new']}`, `mode:${s.strategyMode || 'new'}`).row();
  for (const field of filterFieldsFor(s)) keyboard.text(`${filterLabels[field]}: ${filterValue(s, field)}`, `filter:${field}`).row();
  return keyboard.text('🔙 رجوع', 'panel:settings');
};
const optionKeyboard = (field, current, mode = 'new') => {
  const common = {
    minLiqUsd: [['0','لا يهم'],['2500','2,500$'],['5000','5,000$'],['10000','10,000$'],['25000','25,000$'],['50000','50,000$'],['100000','100,000$']],
    dexBuys: [['0','لا يهم'],['10','10'],['25','25'],['50','50'],['100','100'],['250','250'],['500','500']],
    ratio: [['0','لا يهم'],['1.05','1.05x'],['1.1','1.1x'],['1.2','1.2x'],['1.5','1.5x'],['2','2x']],
    momentum: [['0','لا يهم'],['1','+1%'],['3','+3%'],['5','+5%'],['10','+10%']],
    curve: [['0-100','0% - 100%'],['0-60','0% - 60%'],['0-35','0% - 35%'],['1-60','1% - 60%'],['5-25','5% - 25%'],['10-35','10% - 35%'],['15-50','15% - 50%'],['20-60','20% - 60%']],
    ageMin: [['0','لا يهم'],['15','15 ثانية'],['30','30 ثانية'],['60','1 دقيقة'],['300','5 دقائق'],['600','10 دقائق'],['1800','30 دقيقة'],['3600','ساعة'],['21600','6 ساعات'],['86400','يوم'],['259200','3 أيام'],['604800','7 أيام'],['864000','10 أيام']],
    ageMax: [['0','مفتوح'],['600','10 دقائق'],['1800','30 دقيقة'],['3600','ساعة'],['21600','6 ساعات'],['86400','يوم'],['259200','3 أيام'],['604800','7 أيام'],['864000','10 أيام']],
    mcapMin: [['0','مفتوح'],['10000','10,000$'],['25000','25,000$'],['50000','50,000$'],['100000','100,000$'],['250000','250,000$'],['500000','500,000$']],
    volume: [['0','لا يهم'],['1000','1,000$'],['5000','5,000$'],['10000','10,000$'],['25000','25,000$'],['50000','50,000$'],['100000','100,000$'],['250000','250,000$']],
    buyers: [['0','لا يهم'],['5','5 محافظ'],['10','10 محافظ'],['25','25 محفظة'],['50','50 محفظة'],['100','100 محفظة'],['250','250 محفظة']],
    dev: [['5','≤ 5%'],['10','≤ 10%'],['15','≤ 15%'],['20','≤ 20%'],['25','≤ 25%']],
    top: [['20','≤ 20%'],['25','≤ 25%'],['30','≤ 30%'],['35','≤ 35%'],['40','≤ 40%']],
    watch: [['10','10 دقائق'],['30','30 دقيقة'],['60','ساعة'],['180','3 ساعات'],['360','6 ساعات'],['1440','يوم'],['10080','7 أيام']],
    sl: [['5','-5%'],['10','-10%'],['15','-15%'],['20','-20%'],['25','-25%'],['30','-30%']],
    rugProtection: [['on','مفعل'],['off','معطل']],
    capitalProtection: [['off','معطل'],['10-25','بيع 10% عند -25%'],['20-20','بيع 20% عند -20%'],['30-15','بيع 30% عند -15%'],['50-10','بيع 50% عند -10%'],['100-20','بيع كامل عند -20%']],
    maxHoldTime: [['0','غير مفعل'],['10','10 دقائق'],['30','30 دقيقة'],['60','ساعة'],['180','3 ساعات'],['360','6 ساعات'],['1440','يوم'],['2880','يومان'],['10080','7 أيام']],
    authorities: [['on','إلزامي'],['off','غير إلزامي']],
    liquidity: [['0','لا يهم'],['0.5','0.5 SOL'],['1','1 SOL'],['2.5','2.5 SOL'],['5','5 SOL'],['10','10 SOL'],['30','30 SOL']],
    dominance: [['on','مفعل: الشراء أكبر'],['off','معطل: قبول أي نسبة']],
    allowZeroVolume: [['off','معطل'],['10','مفعل — سيولة ≥ 10 SOL'],['20','مفعل — سيولة ≥ 20 SOL'],['30','مفعل — سيولة ≥ 30 SOL'],['50','مفعل — سيولة ≥ 50 SOL']],
    marketcap: [['0','مفتوح'],['50000','50,000$'],['100000','100,000$'],['250000','250,000$'],['500000','500,000$'],['1000000','1,000,000$'],['10000000','10,000,000$']]
  };
  const growing = {
    ageMin: [['60','1 دقيقة'],['300','5 دقائق'],['900','15 دقيقة'],['1800','30 دقيقة'],['3600','ساعة'],['21600','6 ساعات'],['86400','يوم'],['259200','3 أيام'],['604800','7 أيام'],['864000','10 أيام']],
    ageMax: [['3600','ساعة'],['21600','6 ساعات'],['86400','يوم'],['259200','3 أيام'],['604800','7 أيام'],['864000','10 أيام']],
    volume: [['0','لا يهم'],['5000','5,000$'],['10000','10,000$'],['25000','25,000$'],['50000','50,000$'],['100000','100,000$'],['250000','250,000$'],['500000','500,000$']],
    buyers: [['0','لا يهم'],['5','5 محافظ'],['10','10 محافظ'],['25','25 محفظة'],['50','50 محفظة'],['100','100 محفظة'],['250','250 محفظة'],['500','500 محفظة']],
    dev: [['5','≤ 5%'],['10','≤ 10%'],['15','≤ 15%'],['20','≤ 20%'],['25','≤ 25%']],
    top: [['20','≤ 20%'],['25','≤ 25%'],['30','≤ 30%'],['35','≤ 35%'],['40','≤ 40%']],
    minLiqUsd: [['0','لا يهم'],['2500','2,500$'],['5000','5,000$'],['10000','10,000$'],['25000','25,000$'],['50000','50,000$'],['100000','100,000$'],['250000','250,000$']],
    dexBuys: [['0','لا يهم'],['10','10'],['25','25'],['50','50'],['100','100'],['250','250'],['500','500'],['1000','1,000']],
    ratio: [['0','لا يهم'],['1.05','1.05x'],['1.1','1.1x'],['1.2','1.2x'],['1.5','1.5x'],['2','2x'],['3','3x']],
    mcapMin: [['0','مفتوح'],['10000','10,000$'],['25000','25,000$'],['50000','50,000$'],['100000','100,000$'],['250000','250,000$'],['500000','500,000$'],['1000000','1,000,000$']],
    marketcap: [['0','مفتوح'],['50000','50,000$'],['100000','100,000$'],['250000','250,000$'],['500000','500,000$'],['1000000','1,000,000$'],['5000000','5,000,000$'],['10000000','10,000,000$']],
    watch: [['10','10 دقائق'],['30','30 دقيقة'],['60','ساعة'],['180','3 ساعات'],['360','6 ساعات'],['1440','يوم'],['4320','3 أيام'],['10080','7 أيام']],
    sl: [['5','-5%'],['10','-10%'],['15','-15%'],['20','-20%'],['25','-25%'],['30','-30%']],
    rugProtection: [['on','مفعل'],['off','معطل']],
    capitalProtection: [['off','معطل'],['10-25','بيع 10% عند -25%'],['20-20','بيع 20% عند -20%'],['30-15','بيع 30% عند -15%'],['50-10','بيع 50% عند -10%'],['100-20','بيع كامل عند -20%']],
    maxHoldTime: [['0','غير مفعل'],['30','30 دقيقة'],['60','ساعة'],['180','3 ساعات'],['360','6 ساعات'],['1440','يوم'],['2880','يومان'],['10080','7 أيام']]
  };
  const options = (mode === 'growing' && growing[field]) || common[field] || [['0','لا يهم']];
  const k = new InlineKeyboard();
  options.forEach(([value, label]) => k.text(`${String(current) === value ? '✔ ' : ''}${label}`, `fopt:${field}:${value}`).row());
  return k.text('تأكيد الاختيار ✅', 'fconfirm').text('إلغاء ورجوع 🔙', 'fcancel');
};
...
const filterText = (s) => `⚙️ ${modeLabels[s.strategyMode || 'new']}\n\nالفلاتر المعروضة تخص هذا النظام فقط. لا يتم حفظ التعديل إلا بعد الضغط على تأكيد.\n\n${filterFieldsFor(s).map((field) => `${filterLabels[field]}: ${filterValue(s, field)}`).join('\n')}`;
...
const dailyKeyboard = () => new InlineKeyboard().text('صفقة واحدة', 'set:daily:1').text('صفقتان', 'set:daily:2').row().text('3 صفقات', 'set:daily:3').text('4 صفقات', 'set:daily:4').row().text('صفقات غير محدودة', 'set:daily:0').row().text('رجوع', 'cfg:allocation');
...
function panelText(s) {
  ...
  const networkFeesSol = (s.paperEvents || []).reduce((sum, event) => sum + Number(event.metadata?.networkFeeSol || 0), 0);
  const pnlSign = pnl > 0 ? '+' : '';
  ...
  return [
    '📊 لوحة القنص', '',
    `⚡ الحالة: ${s.autoSniperEnabled ? 'يعمل' : 'متوقف'} | ${s.paperTradingEnabled ? 'شراء ✓' : 'شراء ✗'} | ${s.autoSellEnabled ? 'بيع ✓' : 'بيع ✗'}`, `🧭 النظام النشط: ${modeLabels[s.strategyMode || 'new']}`, '',
    '💰 رأس المال', `├ الرصيد الواقعي: ${lastRealBalanceSol === null ? 'سيُقرأ عند التنفيذ' : `${lastRealBalanceSol.toFixed(6)} SOL`}`, `├ المتاح التجريبي: ${available.toFixed(2)} SOL`, `├ المحجوز: ${reserved.toFixed(2)} SOL`, `├ حجم الصفقة: ${Number(s.paperAllocationPct || 0)}% = ${realTradeAmount(s) === null ? 'يُحسب من الرصيد الواقعي' : `${realTradeAmount(s).toFixed(6)} SOL`}`, `├ [${bar}] ${usedPct}%`, `└ صافي الأرباح: ${pnlSign}${pnl.toFixed(4)} SOL`, '',
    `📈 المراكز المفتوحة: ${openPositions.length}`, `📅 صفقات اليوم: ${Number(s.tradesToday || 0)} / ${Number(s.maxTradesPerDay || 0) > 0 ? Number(s.maxTradesPerDay) : 'مفتوح'}`, `⛓️ رسوم الشبكة المسجلة: ${networkFeesSol.toFixed(9)} SOL`, '',
...
function detailsText(s) {
  ...
  const lines = events.map((e, i) => {
    ...
    if (isBuy) return [
      ...
      `💵 المبلغ: ${Number(m.amountSol || 0).toFixed(4)} SOL`, `⛓️ رسوم شبكة Solana: ${m.networkFeeSol == null ? 'غير مسجلة' : `${Number(m.networkFeeSol).toFixed(9)} SOL`}`, `💧 السيولة: ${Number(m.liquiditySol || 0).toFixed(2)} SOL`,
    ...
    return [
      ...
      `📈 PnL: ${sign}${Number(m.pnlSol || 0).toFixed(4)} SOL (${sign}${pnl.toFixed(1)}%)`, `⛓️ رسوم شبكة Solana: ${m.networkFeeSol == null ? 'غير مسجلة' : `${Number(m.networkFeeSol).toFixed(9)} SOL`}`, `⏱️ المدة: ${mins}د ${secs}ث`,
...
function positionSellKeyboard(values) { const keyboard = new InlineKeyboard(); values.filter((p) => p.status === 'open').forEach((p) => { const token = `m${(++paperTokenCounter).toString(36)}`; manualSellTokens.set(token, p.id); keyboard.text(`💰 بيع ${p.symbol || p.name || shortAddress(p.mint)}`, `manualsell:${token}:ask`).row(); }); if (values.filter((p) => p.status === 'open').length > 1) keyboard.text('💰 بيع جميع العملات المفتوحة', 'manualsell:all:ask').row(); return keyboard; }
function detailsKeyboard(s) { const keyboard = positionSellKeyboard(getPositions(config.adminId, config.encryptionKey).filter((p) => p.status === 'open')); (s.paperEvents || []).slice().reverse().forEach((e, i) => keyboard.copyText(`نسخ عقد ${i + 1}`, e.mint).row()); return keyboard.text('🧹 تنظيف بعد الإيقاف', 'panel:cleanup').row().text('رجوع', 'panel:back'); }
let lastRealBalanceSol = null;
const realTradeAmount = (s) => lastRealBalanceSol === null ? null : Math.max(0, lastRealBalanceSol) * Number(s.paperAllocationPct || 0) / 100;
const paperTradeAmount = (s) => realTradeAmount(s) ?? (Number(s.paperCapitalSol || 1) * Number(s.paperAllocationPct || 10) / 100);
...
async function flushPanelEdit(key) { const entry = panelEditQueue.get(key); if (!entry || entry.running || !entry.pending) return; entry.running = true; const pending = entry.pending; entry.pending = null; const signature = `${pending.text}\n${JSON.stringify(pending.keyboard)}`; if (signature === entry.lastSent) { entry.running = false; return; } try { await bot.api.editMessageText(pending.chatId, pending.messageId, pending.text, { reply_markup: pending.keyboard }); entry.lastSent = signature; } catch (error) { if (error?.error_code === 429 || error?.response?.status === 429) { entry.pending = pending; await new Promise((resolve) => setTimeout(resolve, retryAfterMs(error))); } else if (!String(error.message || '').includes('message is not modified')) console.error(`Panel update error: ${error.message}`); else entry.lastSent = signature; } finally { entry.running = false; if (entry.pending) { clearTimeout(entry.timer); entry.timer = setTimeout(() => flushPanelEdit(key), 500); } } }
function updatePanel(s = settingsForAdmin()) { const chatId = String(s.paperPanelChatId || config.adminId); const messageId = s.paperPanelMessageId; if (!messageId) return Promise.resolve(); const key = `${chatId}:${messageId}`; const entry = panelEditQueue.get(key) || { pending: null, running: false, timer: null, lastSent: null }; entry.pending = { chatId, messageId, text: panelText(s), keyboard: panelKeyboard(s) }; panelEditQueue.set(key, entry); clearTimeout(entry.timer); entry.timer = setTimeout(() => flushPanelEdit(key), 500); return Promise.resolve(); }
async function recordPaperSale(value, sold, reason, triggerType = 'Manual') {
  ...
  s.paperEvents = [...(s.paperEvents || []), { type: 'بيع', name: `${value.name} (${value.symbol})`, mint: value.mint, detail: `${reason} | ${Number(sold.currentSol).toFixed(4)} SOL (${Number(sold.pnlPct).toFixed(1)}%) | رسوم الشبكة ${Number(sold.feeSol || 0).toFixed(9)} SOL`, metadata: { reason, triggerType: autoType, investedSol: Number(value.investedSol || 0), currentSol: Number(sold.currentSol || 0), pnlSol: Number(sold.pnlSol || 0), pnlPct: Number(sold.pnlPct || 0), fraction: Number(sold.fraction || 1), holdSeconds: holdSec, networkFeeLamports: Number(sold.feeLamports || 0), networkFeeSol: Number(sold.feeSol || 0), signature: sold.signature, filtersAtSell: { stopLoss: `-${s.paperStopLossPct}%`, tp1: `+${s.paperTakeProfitFirstPct}%`, tp2: `+${s.paperTakeProfitFinalPct}%`, capitalProtection: s.capitalProtectionEnabled ? `${s.capitalProtectionSellPct}%@-${s.capitalProtectionTriggerPct}%` : 'معطل', maxHoldTime: s.maxHoldTimeMin > 0 ? `${s.maxHoldTimeMin}د` : 'معطل' } }, timestamp: Date.now() }].slice(-20);
...
async function trade(ctx, side, mintArg, amountArg, fraction = null) {
  ...
  try {
    if (side === 'buy') {
      const coin = await watcher.fetchCoin(mint);
      if (!coin) return ctx.reply('مرفوض: لم أستطع قراءة بيانات العملة من المصدر الأساسي. لم تُرسل معاملة.');
      buyCandidate = watcher.normalize(coin);
      await watcher.ensureDexVolume(buyCandidate);
      const filterReason = watcher.filterReason(buyCandidate);
      if (filterReason) return ctx.reply(`مرفوض من الفلتر: ${filterReason}\nلم تُرسل معاملة.`);
      if (effectiveLiveTrading(s) && (buyCandidate.marketDataSource !== 'dexscreener' || !watcher.hasCompleteTradeData(buyCandidate))) return ctx.reply('مرفوض: بيانات العملة الأساسية غير مكتملة أو غير مؤكدة. لم تُرسل معاملة.');
    }
    ...
    if (side === 'buy' && effectiveLiveTrading(s)) {
      ...
      await openPosition({ adminId: config.adminId, key: config.encryptionKey, jupiterUrl: config.jupiterUrl, mint, investedSol: amount, quote: { ...quote, outAmount: actualBalance.raw }, mode: 'live', buySignature: result.signature, metadata: { ...(buyCandidate || {}), name: buyCandidate?.name || mint, symbol: buyCandidate?.symbol || 'N/A', decimals: actualBalance.decimals } });
    }
...
bot.callbackQuery('wallet', async (ctx) => { await ctx.answerCallbackQuery(); if (!isAdmin(ctx)) return ctx.reply('عرض المحفظة متاح للمشرف فقط لحماية بيانات الحساب.'); try { const address = keypairFromSecret(userSecret()).publicKey.toBase58(); const sol = await getSolBalance({ rpcUrl: config.rpcUrl, owner: address }); const portfolio = await refreshPositions({ adminId: config.adminId, key: config.encryptionKey, jupiterUrl: config.jupiterUrl }); const lines = portfolio.values.length ? portfolio.values.map((p) => p.pricingError ? `• ${p.symbol || p.name} — السعر غير متاح` : `• ${p.symbol || p.name} — ${p.currentSol.toFixed(6)} SOL | PnL ${p.pnlPct >= 0 ? '+' : ''}${p.pnlPct.toFixed(1)}% (${p.pnlSol >= 0 ? '+' : ''}${p.pnlSol.toFixed(6)} SOL)`).join('\n') : 'لا توجد عملات مشتراة مفتوحة.'; const keyboard = positionSellKeyboard(portfolio.values); keyboard.row().text('🔄 تحديث المحفظة', 'wallet:refresh').row().text('رجوع للوحة', 'panel:back'); await ctx.editMessageText(`💼 محفظتي\n\nالعنوان:\n${address}\n\nرصيد SOL: ${sol.toFixed(9)}\n\n📈 العملات المشتراة\n${lines}\n\nآخر تحديث: ${new Date().toLocaleString('ar-IQ')}`, { reply_markup: keyboard }); } catch (e) { await ctx.reply(`تعذر تحميل المحفظة.\n${e.message}`); } });
bot.callbackQuery('wallet:refresh', async (ctx) => { await ctx.answerCallbackQuery(); if (!isAdmin(ctx)) return; const address = keypairFromSecret(userSecret()).publicKey.toBase58(); const sol = await getSolBalance({ rpcUrl: config.rpcUrl, owner: address }); const portfolio = await refreshPositions({ adminId: config.adminId, key: config.encryptionKey, jupiterUrl: config.jupiterUrl }); const lines = portfolio.values.length ? portfolio.values.map((p) => p.pricingError ? `• ${p.symbol || p.name} — السعر غير متاح` : `• ${p.symbol || p.name} — ${p.currentSol.toFixed(6)} SOL | PnL ${p.pnlPct >= 0 ? '+' : ''}${p.pnlPct.toFixed(1)}% (${p.pnlSol >= 0 ? '+' : ''}${p.pnlSol.toFixed(6)} SOL)`).join('\n') : 'لا توجد عملات مشتراة مفتوحة.'; const keyboard = positionSellKeyboard(portfolio.values); keyboard.row().text('🔄 تحديث المحفظة', 'wallet:refresh').row().text('رجوع للوحة', 'panel:back'); await ctx.editMessageText(`💼 محفظتي\n\nالعنوان:\n${address}\n\nرصيد SOL: ${sol.toFixed(9)}\n\n📈 العملات المشتراة\n${lines}\n\nآخر تحديث: ${new Date().toLocaleString('ar-IQ')}`, { reply_markup: keyboard }); });
...
bot.callbackQuery(/^filters:(new|growing)$/, async (ctx) => { await ctx.answerCallbackQuery(); if (!isAdmin(ctx)) return; const mode = ctx.match[1]; const s = settingsForAdmin(); applyStrategyMode(s, mode); saveSettings(config.adminId, s, config.encryptionKey); watcher.updateSettings(s); await ctx.editMessageText(`${modeLabels[mode]}\n\n✅ تم تفعيل هذا النظام فقط. النظام الآخر غير فعال حتى تختاره من جديد.\n\n${filterText(s)}`, { reply_markup: filterKeyboard(s) }); });
bot.callbackQuery(/^mode:(new|growing)$/, async (ctx) => { await ctx.answerCallbackQuery(); if (!isAdmin(ctx)) return; const s = settingsForAdmin(); applyStrategyMode(s, ctx.match[1]); saveSettings(config.adminId, s, config.encryptionKey); watcher.updateSettings(s); await ctx.editMessageText(`تم اختيار ${modeLabels[s.strategyMode]}


اضبط قيم هذا الوضع يدويًا من الأزرار ثم شغّل المراقب.`, { reply_markup: filterKeyboard(s) }); });
bot.callbackQuery(/^filter:(minLiqUsd|dexBuys|ratio|momentum|curve|volume|buyers|dev|top|social|watch|authorities|liquidity|dominance|marketcap|ageMin|ageMax|mcapMin|sl|rugProtection|capitalProtection|maxHoldTime|allowZeroVolume)$/, async (ctx) => { await ctx.answerCallbackQuery(); if (!isAdmin(ctx)) return; const field = ctx.match[1]; const s = settingsForAdmin(); if (field === 'curve') { pendingFilters.set(String(ctx.from.id), { field, min: String(s.minCurveProgress), max: String(s.maxCurveProgress) }); return ctx.editMessageText(`${filterLabels.curve}\n\nالقيمة الحالية: ${filterValue(s, 'curve')}\n\nحدد الحد الأدنى أولاً:`, { reply_markup: curveMinKeyboard(String(s.minCurveProgress)) }); } const value = field === 'ageMin' ? String(s.minTokenAgeSec || 0) : field === 'ageMax' ? String(s.maxTokenAgeSec || 0) : field === 'mcapMin' ? String(s.minMarketCapUsd || 0) : field === 'sl' ? String(s.paperStopLossPct || 15) : field === 'rugProtection' ? (s.rugProtectionEnabled ? 'on' : 'off') : field === 'capitalProtection' ? (s.capitalProtectionEnabled ? `${s.capitalProtectionSellPct}-${s.capitalProtectionTriggerPct}` : 'off') : field === 'maxHoldTime' ? String(s.maxHoldTimeMin || 0) : field === 'minLiqUsd' ? String(s.minLiquidityUsd) : field === 'dexBuys' ? String(s.minDexBuys) : field === 'ratio' ? String(s.minBuySellRatio) : field === 'momentum' ? String(s.minPriceChange1hPct) : field === 'volume' ? String(s.minVolumeUsd) : field === 'allowZeroVolume' ? (s.allowZeroVolume ? String(s.allowZeroVolumeMinLiq) : 'off') : field === 'buyers' ? String(s.minUniqueBuyers) : field === 'dev' ? String(s.maxCreatorHoldingsPct) : field === 'top' ? String(s.maxTopHoldersPct) : field === 'watch' ? String(s.watchlistMinutes) : field === 'social' ? (s.requireSocialLinks ? 'on' : 'off') : field === 'authorities' ? (s.requireRenouncedAuthorities ? 'on' : 'off') : field === 'dominance' ? (s.requireBuyVolumeDominance ? 'on' : 'off') : field === 'liquidity' ? String(s.minLiquiditySol || 0) : String(s.maxMarketCapUsd || 0); pendingFilters.set(String(ctx.from.id), { field, value }); await ctx.editMessageText(`${filterLabels[field]}\n\nالقيمة الحالية: ${filterValue(s, field)}\nاختر قيمة جديدة من الأزرار ثم اضغط تأكيد الاختيار.`, { reply_markup: optionKeyboard(field, value, s.strategyMode) }); });
...
bot.callbackQuery(/^fopt:(minLiqUsd|dexBuys|ratio|momentum|curve|volume|buyers|dev|top|social|watch|authorities|liquidity|dominance|marketcap|ageMin|ageMax|mcapMin|sl|rugProtection|capitalProtection|maxHoldTime|allowZeroVolume):(.+)$/, async (ctx) => { await ctx.answerCallbackQuery(); if (!isAdmin(ctx)) return; const field = ctx.match[1]; const value = ctx.match[2]; const s = settingsForAdmin(); pendingFilters.set(String(ctx.from.id), { field, value }); await ctx.editMessageText(`${filterLabels[field]}\n\nالاختيار المؤقت: ${value}\nاضغط تأكيد الاختيار للحفظ أو إلغاء ورجوع.`, { reply_markup: optionKeyboard(field, value, s.strategyMode) }); });
bot.callbackQuery('fconfirm', async (ctx) => { await ctx.answerCallbackQuery(); if (!isAdmin(ctx)) return; const pending = pendingFilters.get(String(ctx.from.id)); if (!pending) return ctx.editMessageText(filterText(settingsForAdmin()), { reply_markup: filterKeyboard(settingsForAdmin()) }); const s = settingsForAdmin(); if (pending.field === 'curve') { const [min, max] = pending.value.split('-').map(Number); s.minCurveProgress = min; s.maxCurveProgress = max; } else if (pending.field === 'ageMin') s.minTokenAgeSec = Number(pending.value); else if (pending.field === 'ageMax') s.maxTokenAgeSec = Number(pending.value); else if (pending.field === 'mcapMin') s.minMarketCapUsd = Number(pending.value); else if (pending.field === 'minLiqUsd') s.minLiquidityUsd = Number(pending.value); else if (pending.field === 'dexBuys') s.minDexBuys = Number(pending.value); else if (pending.field === 'ratio') s.minBuySellRatio = Number(pending.value); else if (pending.field === 'momentum') s.minPriceChange1hPct = Number(pending.value); else if (pending.field === 'volume') s.minVolumeUsd = Number(pending.value); else if (pending.field === 'sl') s.paperStopLossPct = Number(pending.value); else if (pending.field === 'rugProtection') s.rugProtectionEnabled = pending.value === 'on'; else if (pending.field === 'capitalProtection') { if (pending.value === 'off') s.capitalProtectionEnabled = false; else { const [sell, trigger] = pending.value.split('-').map(Number); s.capitalProtectionEnabled = true; s.capitalProtectionSellPct = sell; s.capitalProtectionTriggerPct = trigger; } } else if (pending.field === 'maxHoldTime') s.maxHoldTimeMin = Number(pending.value); else if (pending.field === 'buyers') s.minUniqueBuyers = Number(pending.value); else if (pending.field === 'dev') s.maxCreatorHoldingsPct = Number(pending.value); else if (pending.field === 'top') s.maxTopHoldersPct = Number(pending.value); else if (pending.field === 'social') s.requireSocialLinks = pending.value === 'on'; else if (pending.field === 'watch') s.watchlistMinutes = Number(pending.value); else if (pending.field === 'authorities') s.requireRenouncedAuthorities = pending.value === 'on'; else if (pending.field === 'liquidity') s.minLiquiditySol = Number(pending.value); else if (pending.field === 'dominance') s.requireBuyVolumeDominance = pending.value === 'on'; else if (pending.field === 'marketcap') s.maxMarketCapUsd = Number(pending.value); else if (pending.field === 'allowZeroVolume') { if (pending.value === 'off') s.allowZeroVolume = false; else { s.allowZeroVolume = true; s.allowZeroVolumeMinLiq = Number(pending.value); } } s.filterProfiles = { ...(s.filterProfiles || {}), [s.strategyMode || 'new']: profileSnapshot(s) }; saveSettings(config.adminId, s, config.encryptionKey); watcher.updateSettings(s); pendingFilters.delete(String(ctx.from.id)); await ctx.editMessageText(`تم حفظ ${filterLabels[pending.field]}: ${filterValue(s, pending.field)}\n\nتم تطبيق الإعداد فورياً على المراقب.`, { reply_markup: filterKeyboard(s) }); });
...
bot.callbackQuery('balance:confirm', async (ctx) => { await ctx.answerCallbackQuery(); if (!isAdmin(ctx)) return; await ctx.editMessageText('⚠️ إعادة ضبط الدورة ستعيد الرصيد التجريبي وتصفّر إحصاءات الفحص فقط. ستبقى العملات والمراكز وسجل العمليات محفوظة. هل تريد المتابعة؟', { reply_markup: new InlineKeyboard().text('تأكيد دورة جديدة ✅', 'balance:do').row().text('إلغاء 🔙', 'panel:back') }); });
bot.callbackQuery('balance:do', async (ctx) => { await ctx.answerCallbackQuery(); if (!isAdmin(ctx)) return; const s = settingsForAdmin(); const user = getUser(config.adminId, config.encryptionKey); watcher.resetCycle(); s.paperAvailableSol = Number(s.paperCapitalSol || 0); s.paperPnlSol = 0; s.tradesToday = 0; s.lastMint = null; s.lastFilterResult = 'لم تبدأ الدورة الجديدة بعد'; s.rejectStats = {}; s.checkedCount = 0; s.lastCheckAt = null; saveUser(config.adminId, { ...user, settings: s }, config.encryptionKey); watcher.updateSettings(s); await ctx.editMessageText('✅ تمت إعادة ضبط الإحصاءات والرصيد التجريبي فقط. تم حفظ جميع العملات والمراكز وسجل العمليات، ولم تُرسل أي معاملة حقيقية.', { reply_markup: panelKeyboard(s) }); });
bot.callbackQuery(/^panel:(start|stop|buyon|buyoff|sellon|selloff|liveon|liveoff|liveon_confirm|killon|killoff|refresh|settings|details|back)$/, async (ctx) => { if (!isAdmin(ctx)) return ctx.answerCallbackQuery({ text: 'للمشرف فقط' }); const action = ctx.match[1]; try { await ctx.answerCallbackQuery({ text: action === 'start' ? 'جارٍ التشغيل...' : 'تم' }); } catch (_) {} const s = settingsForAdmin(); s.paperPanelChatId = String(ctx.callbackQuery.message.chat.id); s.paperPanelMessageId = ctx.callbackQuery.message.message_id; if (action === 'liveon') return ctx.editMessageText('⚠️ التداول الحقيقي سيرسل معاملات فعلية إلى شبكة Solana ويعرّض الأموال للخطر. اختر تأكيدًا صريحًا للمتابعة:', { reply_markup: new InlineKeyboard().text('تأكيد تشغيل التداول الحقيقي', 'panel:liveon_confirm').row().text('إلغاء', 'panel:back') }); try { if (action === 'start') { s.killSwitch = false; watcherManuallyEnabled = true; s.autoSniperEnabled = true; watcher.updateSettings(s); watcher.start(); } if (action === 'stop') { watcherManuallyEnabled = false; s.autoSniperEnabled = false; watcher.updateSettings(s); watcher.stop(); } if (action === 'buyon') s.paperTradingEnabled = true; if (action === 'buyoff') s.paperTradingEnabled = false; if (action === 'sellon') s.autoSellEnabled = true; if (action === 'selloff') s.autoSellEnabled = false; if (action === 'liveon_confirm') { if (String(process.env.LIVE_TRADING).toLowerCase() !== 'true') return ctx.editMessageText('لم يتم التشغيل: متغير LIVE_TRADING في Render ليس true. التداول الحقيقي يبقى مغلقاً افتراضياً.', { reply_markup: new InlineKeyboard().text('رجوع للوحة', 'panel:back') }); s.liveTrading = true; } if (action === 'liveoff') s.liveTrading = false; if (action === 'killon') { s.killSwitch = true; s.autoSniperEnabled = false; s.paperTradingEnabled = false; s.autoSellEnabled = false; watcherManuallyEnabled = false; watcher.updateSettings(s); watcher.stop(); } if (action === 'killoff') { s.killSwitch = false; s.autoSniperEnabled = true; watcherManuallyEnabled = true; watcher.updateSettings(s); watcher.start(); } saveSettings(config.adminId, s, config.encryptionKey); if (action === 'refresh') return ctx.editMessageText(panelText(s), { reply_markup: panelKeyboard(s) }); if (action === 'settings') return ctx.editMessageText('ضبط القنص التجريبي\nاختر الإعداد المطلوب:', { reply_markup: settingsKeyboard() }); if (action === 'details') return ctx.editMessageText(detailsText(s), { reply_markup: detailsKeyboard(s) }); if (action === 'back') return ctx.editMessageText(panelText(s), { reply_markup: panelKeyboard(s) }); await ctx.editMessageText(panelText(s), { reply_markup: panelKeyboard(s) }); } catch (error) { console.error(`Panel button ${action} error: ${error.stack || error.message}`); try { await ctx.reply(`تعذر تنفيذ الزر حالياً: ${error.message}`); } catch (_) {} } });
bot.callbackQuery(/^cfg:(allocation|daily|profit|sizing|confirm)$/, async (ctx) => { try { await ctx.answerCallbackQuery(); } catch (_) {} if (!isAdmin(ctx)) return; const page = ctx.match[1]; if (page === 'allocation') { const address = keypairFromSecret(userSecret()).publicKey.toBase58(); const balance = await getSolBalance({ rpcUrl: config.rpcUrl, owner: address }); return ctx.editMessageText(`اختر نسبة الصفقة من رصيد محفظتك الواقعي: ${balance.toFixed(6)} SOL\n\nالحساب: الرصيد الكامل × النسبة ÷ 100\nرسوم الشبكة: تُحسب وتُسجل من المعاملة الفعلية فقط، ولا يفرض البوت أي رسوم.`, { reply_markup: allocationKeyboard() }); } if (page === 'daily') return ctx.editMessageText('اختر الحد الأقصى للصفقات في اليوم:', { reply_markup: dailyKeyboard() }); if (page === 'profit') return ctx.editMessageText('اختر نسبة الربح التي عندها يتم البيع التلقائي:', { reply_markup: profitKeyboard() }); if (page === 'sizing') return ctx.editMessageText('اختر طريقة حساب رأس المال:', { reply_markup: sizingKeyboard() }); const s = settingsForAdmin(); saveSettings(config.adminId, s, config.encryptionKey); watcher.updateSettings(s); await ctx.editMessageText(settingsText(s), { reply_markup: settingsKeyboard() }); });
 bot.callbackQuery(/^set:(allocation|daily|profit|sizing):(.+)$/, async (ctx) => { await ctx.answerCallbackQuery(); if (!isAdmin(ctx)) return; const [, key, value] = ctx.callbackQuery.data.split(':'); const s = settingsForAdmin(); if (key === 'allocation') { s.paperAllocationPct = Number(value); saveSettings(config.adminId, s, config.encryptionKey); const address = keypairFromSecret(userSecret()).publicKey.toBase58(); const balance = await getSolBalance({ rpcUrl: config.rpcUrl, owner: address }); const amount = balance * s.paperAllocationPct / 100; return ctx.editMessageText(`تم حفظ نسبة الصفقة: ${s.paperAllocationPct}%\nالرصيد الواقعي: ${balance.toFixed(6)} SOL\nالمبلغ المحسوب للصفقة: ${amount.toFixed(6)} SOL\nالحساب: ${balance.toFixed(6)} × ${s.paperAllocationPct} ÷ 100\nرسوم الشبكة: تُحسب وتُسجل من المعاملة الفعلية فقط، ولا يفرض البوت أي رسوم.\n\nاختر عدد الصفقات اليومية:`, { reply_markup: dailyKeyboard() }); } if (key === 'daily') { const dailyLimit = Number(value); if (![0, 1, 2, 3, 4].includes(dailyLimit)) return ctx.editMessageText('اختر حدًا من 1 إلى 4 صفقات أو صفقات غير محدودة.', { reply_markup: dailyKeyboard() }); s.maxTradesPerDay = dailyLimit; saveSettings(config.adminId, s, config.encryptionKey); return ctx.editMessageText(`تم حفظ الحد اليومي: ${dailyLimit === 0 ? 'غير محدود' : `${dailyLimit} ${dailyLimit === 1 ? 'صفقة' : 'صفقات'}`}.\nاختر هدف الربح والبيع:`, { reply_markup: profitKeyboard() }); } if (key === 'profit') { s.paperTakeProfitPct = Number(value); saveSettings(config.adminId, s, config.encryptionKey); return ctx.editMessageText('تم حفظ هدف الربح. اختر نمط حجم الصفقة:', { reply_markup: sizingKeyboard() }); } s.paperSizingMode = value; saveSettings(config.adminId, s, config.encryptionKey); await ctx.editMessageText('تم حفظ نمط الصفقة. القنص متوقف؛ استخدم زر تشغيل القنص من اللوحة عند الحاجة.', { reply_markup: settingsKeyboard() }); });
...
const watcher = new PumpFunWatcher({ adminId: config.adminId, settings: settingsForAdmin(), onError: (e) => console.error(`Pump.fun watcher error: ${e.message}`), onFilter: (_candidate, reason) => { const key = reason.replace(/—.*$/, '').trim().slice(0, 40); _rejectBuf[key] = (_rejectBuf[key] || 0) + 1; _checkedBuf += 1; }, onCandidate: async (candidate) => {
  ...
  try {
    ...
    if (effectiveLiveTrading(s)) {
      ...
      if (liveFilterReason || candidate.marketDataSource !== 'dexscreener' || !watcher.hasCompleteTradeData(candidate)) {
        console.warn(`[sniper] live buy blocked for ${candidate.mint}: ${liveFilterReason || 'بيانات العملة الأساسية غير مكتملة'}`);
        return;
      }
    }
    ...
    s = settingsForAdmin();
    const livePosition = await openPosition({ adminId: config.adminId, key: config.encryptionKey, jupiterUrl: config.jupiterUrl, mint: candidate.mint, investedSol: amountSol, quote: { ...quote, outAmount: actualBalance.raw }, mode: 'live', buySignature: result.signature, metadata: { ...candidate, networkFeeLamports: result.feeLamports, networkFeeSol: result.feeSol } });
    if (!livePosition) throw new Error('تعذر فتح سجل المركز الحي.');
    s.tradesToday += 1; resetConsecutiveFailures(s);
    s.paperEvents = [...(s.paperEvents || []), { type: 'شراء', name: `${candidate.name} (${candidate.symbol})`, mint: candidate.mint, detail: `${amountSol.toFixed(4)} SOL | رسوم الشبكة ${Number(result.feeSol || 0).toFixed(9)} SOL`, metadata: { amountSol: Number(amountSol), networkFeeLamports: Number(result.feeLamports || 0), networkFeeSol: Number(result.feeSol || 0), signature: result.signature, mode: 'live' }, timestamp: Date.now() }].slice(-20);
    saveSettings(config.adminId, s, config.encryptionKey);
    const status = `تم التنفيذ\nرسوم شبكة Solana: ${Number(result.feeSol || 0).toFixed(9)} SOL\n${explorer(result.signature)}`;
