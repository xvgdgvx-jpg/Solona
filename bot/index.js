...
const liveReserveSol = () => Math.max(0, Number(process.env.LIVE_SOL_RESERVE_SOL || 0.01));
...
const watcher = new PumpFunWatcher({ adminId: config.adminId, settings: settingsForAdmin(), onError: (e) => console.error(`Pump.fun watcher error: ${e.message}`), onFilter: (_candidate, reason) => { const key = reason.replace(/—.*$/, '').trim().slice(0, 40); _rejectBuf[key] = (_rejectBuf[key] || 0) + 1; _checkedBuf += 1; }, onCandidate: async (candidate) => {
  ...
  try {
    ...
    if (effectiveLiveTrading(s) && lastRealBalanceSol < amountSol + liveReserveSol()) {
      console.log(`[sniper] تخطي الشراء: الرصيد ${lastRealBalanceSol.toFixed(6)} SOL لا يغطي الصفقة ${amountSol.toFixed(6)} SOL واحتياطي رسوم الشبكة ${liveReserveSol().toFixed(6)} SOL`);
      return;
    }
