...
bot.callbackQuery(/^filter:(curve|volume|buyers|dev|top|social|watch|authorities|liquidity|dominance|marketcap|ageMin|ageMax|mcapMin|sl|capitalProtection|maxHoldTime)$/, async (ctx) => { await ctx.answerCallbackQuery(); if (!isAdmin(ctx)) return; const field = ctx.match[1]; const s = settingsForAdmin(); if (field === 'curve') { pendingFilters.set(String(ctx.from.id), { field, min: String(s.minCurveProgress), max: String(s.maxCurveProgress) }); return ctx.editMessageText(`${filterLabels.curve}\n\nالقيمة الحالية: ${filterValue(s, 'curve')}\n\nحدد الحد الأدنى أولاً:`, { reply_markup: curveMinKeyboard(String(s.minCurveProgress)) }); } const value = field === 'ageMin' ? String(s.minTokenAgeSec || 0) : field === 'ageMax' ? String(s.maxTokenAgeSec || 0) : field === 'mcapMin' ? String(s.minMarketCapUsd || 0) : field === 'sl' ? String(s.paperStopLossPct || 15) : field === 'capitalProtection' ? (s.capitalProtectionEnabled ? `${s.capitalProtectionSellPct}-${s.capitalProtectionTriggerPct}` : 'off') : field === 'maxHoldTime' ? String(s.maxHoldTimeMin || 0) : field === 'volume' ? String(s.minVolumeUsd) : field === 'buyers' ? String(s.minUniqueBuyers) : field === 'dev' ? String(s.maxCreatorHoldingsPct) : field === 'top' ? String(s.maxTopHoldersPct) : field === 'watch' ? String(s.watchlistMinutes) : field === 'social' ? (s.requireSocialLinks ? 'on' : 'off') : field === 'authorities' ? (s.requireRenouncedAuthorities ? 'on' : 'off') : field === 'dominance' ? (s.requireBuyVolumeDominance ? 'on' : 'off') : field === 'liquidity' ? String(s.minLiquiditySol || 0) : String(s.maxMarketCapUsd || 0); pendingFilters.set(String(ctx.from.id), { field, value }); await ctx.editMessageText(`${filterLabels[field]}\n\nالقيمة الحالية: ${filterValue(s, field)}\nاختر قيمة جديدة ثم اضغط تأكيد الاختيار.`, { reply_markup: optionKeyboard(field, value) }); });
