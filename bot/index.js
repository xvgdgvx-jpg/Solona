...
const filterLabels = { curve: '📉 نسبة المنحنى', volume: '💵 الحد الأدنى للحجم', buyers: '👥 الحد الأدنى للمشترين', dev: '👨‍💻 أقصى نسبة للمطور', top: '🐋 كبار الملاك', social: '🔗 روابط التواصل', authorities: '🔒 تأمين العقد والسيولة', liquidity: '💧 الحد الأدنى لسيولة SOL', dominance: '📊 تفوق الشراء على البيع', marketcap: '🎯 سقف القيمة السوقية', watch: '⏱️ مدة المراقبة' };
const filterValue = (s, field) => ({ curve: `${s.minCurveProgress}-${s.maxCurveProgress}%`, volume: `$${s.minVolumeUsd}`, buyers: `${s.minUniqueBuyers} محافظ`, dev: `${s.maxCreatorHoldingsPct}%`, top: `${s.maxTopHoldersPct}%`, social: s.requireSocialLinks ? 'إلزامية' : 'غير إلزامية', watch: `${s.watchlistMinutes} دقائق`, authorities: s.requireRenouncedAuthorities ? 'إلزامي' : 'غير إلزامي', liquidity: s.minLiquiditySol ? `${s.minLiquiditySol} SOL` : 'لا يهم', dominance: s.requireBuyVolumeDominance ? 'مفعل' : 'معطل', marketcap: s.maxMarketCapUsd ? `$${s.maxMarketCapUsd}` : 'مفتوح' }[field]);
const filterKeyboard = (s) => new InlineKeyboard().text(`${filterLabels.curve}: ${filterValue(s, 'curve')}`, 'filter:curve').row().text(`${filterLabels.volume}: ${filterValue(s, 'volume')}`, 'filter:volume').row().text(`${filterLabels.buyers}: ${filterValue(s, 'buyers')}`, 'filter:buyers').row().text(`${filterLabels.dev}: ${filterValue(s, 'dev')}`, 'filter:dev').row().text(`${filterLabels.top}: ${filterValue(s, 'top')}`, 'filter:top').row().text(`${filterLabels.social}: ${filterValue(s, 'social')}`, 'filter:social').row().text(`${filterLabels.authorities}: ${filterValue(s, 'authorities')}`, 'filter:authorities').row().text(`${filterLabels.liquidity}: ${filterValue(s, 'liquidity')}`, 'filter:liquidity').row().text(`${filterLabels.dominance}: ${filterValue(s, 'dominance')}`, 'filter:dominance').row().text(`${filterLabels.marketcap}: ${filterValue(s, 'marketcap')}`, 'filter:marketcap').row().text(`${filterLabels.watch}: ${filterValue(s, 'watch')}`, 'filter:watch').row().text('🔙 رجوع', 'panel:settings');
const optionKeyboard = (field, current) => { const options = { curve: [['5-25','5% - 25%'],['10-35','10% - 35%'],['15-50','15% - 50%'],['20-60','20% - 60%']], volume: [['500','500$'],['1000','1,000$'],['1500','1,500$'],['2500','2,500$'],['5000','5,000$']], buyers: [['5','5 محافظ'],['10','10 محافظ'],['15','15 محفظة'],['25','25 محفظة']], dev: [['5','أقصى حد 5%'],['10','أقصى حد 10%'],['20','أقصى حد 20%'],['25','أقصى حد 25%']], top: [['15','15%'],['20','20%'],['25','25%'],['30','30%']], social: [['on','إلزامي'],['off','غير إلزامي']], watch: [['1','1 دقيقة'],['3','3 دقائق'],['5','5 دقائق'],['10','10 دقائق']], authorities: [['on','إلزامي (Renounced)'],['off','غير إلزامي']], liquidity: [['0.5','0.5 SOL'],['1','1.0 SOL'],['2.5','2.5 SOL'],['5','5.0 SOL'],['0','لا يهم']], dominance: [['on','مفعل (الشراء أكبر من البيع)'],['off','معطل (قبول أي نسبة)']], marketcap: [['30000','30,000$'],['60000','60,000$'],['100000','100,000$'],['0','مفتوح']] }[field]; const k = new InlineKeyboard(); options.forEach(([value, label]) => k.text(`${String(current) === value ? '✔ ' : ''}${label}`, `fopt:${field}:${value}`).row()); return k.text('تأكيد الاختيار ✅', 'fconfirm').text('إلغاء ورجوع 🔙', 'fcancel'); };
const curveMinKeyboard = (current) => { const k = new InlineKeyboard(); [['5','5٪'],['10','10٪'],['15','15٪'],['20','20٪']].forEach(([v, label]) => k.text(`${current === v ? '✔ ' : ''}${label}`, `fmin:${v}`).row()); return k.text('رجوع', 'fcancel'); };
const curveMaxKeyboard = (min) => { const k = new InlineKeyboard(); [['25','25٪'],['35','35٪'],['50','50٪'],['60','60٪']].filter(([v]) => Number(v) > Number(min)).forEach(([v, label]) => k.text(`${label}`, `fmax:${min}:${v}`).row()); return k.text('رجوع', 'fcancel'); };
...
bot.callbackQuery('wallet', async (ctx) => { await ctx.answerCallbackQuery(); if (!isAdmin(ctx)) return ctx.reply('عرض المحفظة متاح للمشرف فقط لحماية بيانات الحساب.'); await ctx.reply('استخدم /wallet لعرض العنوان.'); });
bot.callbackQuery('portfolio', async (ctx) => { await ctx.answerCallbackQuery(); if (!isAdmin(ctx)) return ctx.reply('عرض الأرصدة متاح للمشرف فقط.'); await ctx.reply('استخدم /portfolio لعرض الأرصدة.'); });
...
bot.callbackQuery(/^watcher:(on|off|status)$/, async (ctx) => { await ctx.answerCallbackQuery(); if (!isAdmin(ctx)) return; const action = ctx.callbackQuery.data.split(':')[1]; const s = settingsForAdmin(); if (action === 'on') { s.autoSniperEnabled = true; watcher.updateSettings(s); watcher.start(); } if (action === 'off') { s.autoSniperEnabled = false; watcher.updateSettings(s); watcher.stop(); } s.paperPanelChatId = String(ctx.chat.id); s.paperPanelMessageId = ctx.callbackQuery.message.message_id; saveSettings(config.adminId, s, config.encryptionKey); await updatePanel(s); });
...
const watcher = new PumpFunWatcher({ adminId: config.adminId, settings: settingsForAdmin(), onError: (e) => console.error(`Pump.fun watcher error: ${e.message}`), onFilter: (candidate, reason) => { const s = settingsForAdmin(); s.lastFilterResult = `${candidate.symbol || candidate.mint}: مرفوض — ${reason}`; s.rejectStats = s.rejectStats || {}; const key = reason.split('—')[0].trim().slice(0, 40); s.rejectStats[key] = (s.rejectStats[key] || 0) + 1; s.checkedCount = (s.checkedCount || 0) + 1; s.lastCheckAt = new Date().toISOString(); saveSettings(config.adminId, s, config.encryptionKey); updatePanel(s); }, onCandidate: async (candidate) => {
