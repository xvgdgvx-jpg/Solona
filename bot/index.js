...
bot.callbackQuery(/^panel:(start|stop|buyoff|sellon|refresh|settings|details|back)$/, async (ctx) => { await ctx.answerCallbackQuery(); if (!isAdmin(ctx)) return; const s = settingsForAdmin(); const action = ctx.match[1]; s.paperPanelChatId = String(ctx.chat.id); s.paperPanelMessageId = ctx.callbackQuery.message.message_id; if (action === 'start') { s.autoSniperEnabled = true; s.paperTradingEnabled = true; watcher.updateSettings(s); watcher.start(); } if (action === 'stop') { s.autoSniperEnabled = false; s.paperTradingEnabled = false; watcher.updateSettings(s); watcher.stop(); } if (action === 'buyoff') { s.paperTradingEnabled = false; } if (action === 'sellon') { s.paperTradingEnabled = true; } saveSettings(config.adminId, s, config.encryptionKey); if (action === 'settings') return ctx.editMessageText('ضبط القنص التجريبي\nاختر الإعداد المطلوب:', { reply_markup: settingsKeyboard() }); if (action === 'details') return ctx.editMessageText(detailsText(s), { reply_markup: new InlineKeyboard().text('رجوع', 'panel:back') }); if (action === 'back') return ctx.editMessageText(panelText(s), { reply_markup: panelKeyboard(s) }); await ctx.editMessageText(panelText(s), { reply_markup: panelKeyboard(s) }); });
