...
async function monitorPaperPositions() {
  ...
  try {
    ...
    for (const value of values) {
      ...
      if (!Number.isFinite(takeProfitPct) || takeProfitPct <= 0 || value.pnlPct < takeProfitPct) continue;
      const reason = `جني ربح +${takeProfitPct}% — إغلاق المركز بالكامل`;
