...
class DexWatcher {
  ...
  async evaluate(candidate) {
    ...
    if (dex.antiDuplicate !== false) { const { getPositions } = require('./paper'); const activeMode = this.settings.paperTradingEnabled === false && this.settings.liveTrading === true ? 'live' : 'paper'; const openPositions = getPositions(this.adminId, this.encryptionKey).filter((p) => p.status === 'open' && (p.mode || 'paper') === activeMode); if (openPositions.some((p) => String(p.mint || '').trim() === String(candidate.mint).trim())) return fail(`عملة مكررة (${candidate.mint})`, 'dex'); }
